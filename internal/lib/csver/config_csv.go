...
func CreateConfigCsv(item *Csv) *ConfigCsv {
	...
	// 去掉空的列
	colHasVal := helper.GridDoesColumnHasValue(dataGrid)        // 判断列是否有效(至少要包含一个值)
	dataGrid = helper.GridFilterColumn(dataGrid, colHasVal)     // 删去空列
	headerGrid = helper.GridFilterColumn(headerGrid, colHasVal) // 删去空列
